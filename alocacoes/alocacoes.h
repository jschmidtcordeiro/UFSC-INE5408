...
std::vector<Aluno> *turma(std::string nomes[], int matriculas[], int N) {
    std::vector<Aluno> *t;
...
*/
std::vector<Aluno> *turmas_uniao(Aluno t1[], Aluno t2[], int N1, int N2) {
    std::vector<Aluno> *tu;
    int NU = N1 + N2;

    for (int i = 0; i < NU; i++) {
        tu[i].escreveNome(nomes[i]);
        tu[i].escreveMatricula(matriculas[i]);
    }
...
inicialmente são iguais a 'nullptr'), a primeira com k elementos e a segunda com o restante (N-k elementos); exemplo:
...
void turmas_divisao(Aluno t[], int k, int N, Aluno **pt1, Aluno **pt2) {
    ...
    for (int i = 0; i < N; i++) {
        if (i < k) {
            t1[i].escreveNome(nomes[i]);
            t1[i].escreveMatricula(matriculas[i]);
        }
        else {
            t2[i - k].escreveNome(nomes[i]);
            t2[i - k].escreveMatricula(matriculas[i]);
        }
    }
